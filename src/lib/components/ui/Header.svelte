...
{#if links.length > 0}
	<header>
		<div class="map_info">
			For accomodation, please fill this form: <a
		...
		<div class="actual_nav" style="width: 100%; height:100%;display:flex;">
						...
						<a
							...
							on:click={() => {
								if (link.linkText == 'Workshop') {
									...
									displayPopUp('Message', 'Coming Soon', 5000, () => {});
								}
							}}
							on:mouseleave={() => {
								setTimeout(() => {
			...
			<a
				class="account"
				href={$invalidate || !$loggedIn ? `/login/?to=${$page.url.pathname}` : '/profile'}
			>
	...
	<div
		class="popup-menu {showMenu ? 'show' : 'hide'}"
		style={`top: ${popupMenuPosition.top}px; right: ${popupMenuPosition.right}px;`}
	>
		<nav class="popup-nav">
			{#each links as link}
				<a href={link.url} aria-label={link.linkText} on:click={() => (showMenu = false)}
					>{link.linkText}</a
				>
				{#each link.childLinks as otherLink}
					<a href={otherLink.url} aria-label={otherLink.linkText}>{otherLink.linkText}</a>
...
<style>
	...
	.account {
		...
		transition: transform 300ms cubic-bezier(0.075, 0.82, 0.165, 1),
			filter 300ms cubic-bezier(0.075, 0.82, 0.165, 1);
	}
